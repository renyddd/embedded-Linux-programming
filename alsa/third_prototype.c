...
int main()
{
    ...
    time_t tmp;
    tmp = time(NULL);
    mytime = localtime((time_t *)&time);

    char wav_file_dir[100] = {"./record_repository/test.wav\0"};

    char wav_file_name[100] = {"\0"};
    sprintf(wav_file_name, "%d", mytime->tm_sec);

    wav_file_name = strcat(wav_file_dir, wav_file_name);
    char *arg_list[] = {
        "arecord", "-r",
        "16000",
        "-t",
